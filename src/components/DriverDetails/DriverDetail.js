...
const DriverDetail = () => {
  ...
  useEffect(() => {
    const fetchDrivers = async () => {
      ...
      const driversWithImageUrls = await Promise.all(
        data.map(async (driver) => {
          let signedUrl = "";

          if (driver.DriverPhoto) {
            // Extract the internal path from the full public URL
            const urlParts = driver.DriverPhoto.split("/driver-photos/");
            const filePath = urlParts[1]; // 'driver-photos/1749749606240.jfif'

            if (!filePath) {
              console.warn("Invalid DriverPhoto path:", driver.DriverPhoto);
              return driver;
            }
          ...
          return {
            ...driver,
            DriverPhotoUrl: signedUrl,
          };
        })
      );

      setDrivers(driversWithImageUrls);
    };
    ...
    <div className="p-6 w-full">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div
            onClick={addNew}
            className="justify-between p-2 pr-20 pl-20 rounded-lg flex items-center custom-button m-2"
          >
            <span>Add New</span>
          </div>
          ...
          {currentDrivers.map((driver) => (
            <div>
              <div key={driver.id} className="driver-card">
                <div>
                  <img
                    src={driver.DriverPhotoUrl || Driver} // Fallback if not found
                    alt="Driver"
